...
from HEPCNN.dataset_nanoAOD import NanoAODDataset as MyDataset
...
try:
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(tqdm(trnLoader, desc='epoch %d/%d' % (epoch+1, args.epoch))):
            if data.weight.shape[0] != data.batch[-1].item()+1:
                print("batch mismatch", i, label.shape[0], pred.shape[0], data.batch[-1].item()+1)
                continue
